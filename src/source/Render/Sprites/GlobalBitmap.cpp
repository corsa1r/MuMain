...
namespace
{
    ...
    // --- Normal-map sibling loading (per-pixel lighting) -------------------
    // A diffuse <base>.OZJ/.OZT may have a sibling <base>_n.OZJ/.OZT holding a
    // tangent-space normal map (generated by tools/upscale/gen_normals.ps1).
    // These load it into a fresh GL texture (0 = absent). Generated normals are
    // the SAME power-of-two dimensions as the diffuse, so no POT padding/UV
    // remap is needed. The texture is left bound; callers rebind as usual.
    constexpr int kNormalMaxDim = 4096;

    std::wstring NormalSibling(const std::wstring& path)
    {
        const std::size_t dot = path.find_last_of(L'.');
        if (dot == std::wstring::npos) return path + L"_n";
        return path.substr(0, dot) + L"_n" + path.substr(dot);
    }

    GLuint UploadNormalTexture(const unsigned char* pixels, int w, int h,
                               GLenum fmt, int comps, GLuint uiFilter, GLuint uiWrapMode)
    {
        GLuint tex = 0;
        glGenTextures(1, &tex);
        glBindTexture(GL_TEXTURE_2D, tex);
        glTexImage2D(GL_TEXTURE_2D, 0, comps, w, h, 0, fmt, GL_UNSIGNED_BYTE, pixels);
    ...
    GLuint LoadNormalOzj(const std::wstring& diffuseOzjPath, GLuint uiFilter, GLuint uiWrapMode)
    {
        std::ifstream f(NarrowPath(NormalSibling(diffuseOzjPath)), std::ios::binary);
        if (!f) return 0;
        std::vector<unsigned char> buf((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
        f.close();
        if (buf.size() <= 24) return 0;

        const unsigned char* jpeg = buf.data() + 24;
        const auto jpegSize = static_cast<unsigned long>(buf.size() - 24);
        int w = 0, h = 0, ss = TJSAMP_444, cs = TJCS_RGB;
        TurboJpegHandle tj;
        if (!tj.valid()) return 0;
        if (tjDecompressHeader3(tj.get(), jpeg, jpegSize, &w, &h, &ss, &cs) != 0
            || w <= 0 || h <= 0 || w > kNormalMaxDim || h > kNormalMaxDim)
            return 0;

        std::vector<unsigned char> rgb(static_cast<std::size_t>(w) * h * 3u);
        if (tjDecompress2(tj.get(), jpeg, jpegSize, rgb.data(), w, 0, h, TJPF_RGB, TJFLAG_FASTDCT) != 0)
            return 0;
        return UploadNormalTexture(rgb.data(), w, h, GL_RGB, 3, uiFilter, uiWrapMode);
    }

    GLuint LoadNormalOzt(const std::wstring& diffuseOztPath, GLuint uiFilter, GLuint uiWrapMode)
    {
        std::ifstream f(NarrowPath(NormalSibling(diffuseOztPath)), std::ios::binary);
        if (!f) return 0;
        std::vector<unsigned char> pak((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
        f.close();
        if (pak.size() < 22) return 0;

        int index = 16;
        std::int16_t nx, ny;
        std::memcpy(&nx, &pak[index], sizeof(nx)); index += 2;
        std::memcpy(&ny, &pak[index], sizeof(ny)); index += 2;
        const char bit = pak[index]; index += 1; index += 1;
        if (bit != 32 || nx <= 0 || ny <= 0 || nx > kNormalMaxDim || ny > kNormalMaxDim) return 0;
        if (static_cast<std::size_t>(22) + static_cast<std::size_t>(nx) * ny * 4u > pak.size()) return 0;

        std::vector<unsigned char> rgba(static_cast<std::size_t>(nx) * ny * 4u);
        for (int y = 0; y < ny; y++)
        {
            const unsigned char* src = &pak[index]; index += nx * 4;
            unsigned char* dst = &rgba[static_cast<std::size_t>(ny - 1 - y) * nx * 4];
            for (int x = 0; x < nx; x++)
            {
        ...
        return UploadNormalTexture(rgba.data(), nx, ny, GL_RGBA, 4, uiFilter, uiWrapMode);
    }
}
...
void CGlobalBitmap::UnloadImage(GLuint uiBitmapIndex, bool bForce)
{
    ...
    if (mi != m_mapBitmap.end())
    {
        ...
        if (--pBitmap->Ref == 0 || bForce)
        {
            ...
            if (pBitmap->NormalTextureNumber)
                glDeleteTextures(1, &(pBitmap->NormalTextureNumber));
...
bool CGlobalBitmap::OpenJpegTurbo(GLuint uiBitmapIndex, const std::wstring& filename, GLuint uiFilter, GLuint uiWrapMode)
{
    ...
    BITMAP_t* rawBmp = pNewBitmap.get();
    ...
    // Load the sibling normal map (per-pixel lighting). Done last so the diffuse
    // texture's parameters above are unaffected; leaves the normal texture bound
    // (callers rebind the diffuse via BindTexture before drawing).
    rawBmp->NormalTextureNumber = LoadNormalOzj(filename_ozj, uiFilter, uiWrapMode);
...
bool CGlobalBitmap::OpenTga(GLuint uiBitmapIndex, const std::wstring& filename, GLuint uiFilter, GLuint uiWrapMode)
{
    ...
    // Sibling normal map (see OpenJpegTurbo for rationale).
    rawBmp->NormalTextureNumber = LoadNormalOzt(filename_ozt, uiFilter, uiWrapMode);
